...
interface Props { content: ContentResponse; onDelete?: (id: number) => void }
  ...
  return (
    <article className="card hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-gray-100">
            ...
            <div className="absolute bottom-full left-0 mb-2 bg-white rounded-2xl shadow-lg border border-gray-100 flex gap-1 p-2 z-10">
              {REACTIONS.map(r => (
                <button key={r.type} onClick={() => handleReact(r.type)}
                  className={`text-xl p-1.5 rounded-xl hover:bg-gray-100 transition-colors ${content.myReaction === r.type ? 'bg-pink-50' : ''}`}
                  title={r.type}>
                  {r.emoji}
                </button>
              ))}
        ...
        <Link to={`/content/${content.id}#comments`}
          className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-happy-600">
          💬 {content.commentsCount}
        </Link>

        {(user?.id === content.author.id || user?.role === 'MODERATOR' || user?.role === 'ADMIN') && onDelete && (
          <button onClick={() => onDelete(content.id)}
            className="ml-auto text-xs text-red-400 hover:text-red-600">🗑️ Elimina</button>
        )}
      </div>
    </article>
  )
