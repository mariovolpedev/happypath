...
export default function ContentCard({content: initial, onDelete}: Props) {
    ...
    const displayAuthor = content.alterEgo
        ? {
            ...content.author,
            displayName: `${content.alterEgo.name}`,
            avatarUrl: content.alterEgo.avatarUrl,
        }
        : content.author
    ...
    const handleReact = async (type: string) => {
        ...
        if (content.myReaction === type) {
            ...
            setContent((c) => ({
                ...
                reactionsByType: {
                    ...c.reactionsByType,
                    [type]: (c.reactionsByType[type] ?? 1) - 1,
                },
            }))
        } else {
            ...
            <article className="card hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start justify-between mb-3">
                    ...
                    >
                        ...
                        <div>
                            <div
                                className="flex items-center gap-1 font-semibold text-gray-800 group-hover:text-happy-600 leading-tight">
                    ...
                    <div className="flex items-center gap-2">
                        {content.theme && (
                            <span
                                className="text-xs bg-amber-100 text-amber-700 rounded-full px-2.5 py-1 flex items-center gap-1">
            {content.theme.iconEmoji} {content.theme.name}
                    ...
                    {content.dedications?.length > 0 && (
                        <p className="text-xs text-happy-600 mb-2 italic">
                            💌 Dedicato a{' '}
                            {content.dedications.map((d) => d.to.displayName).join(', ')}
                        </p>
                    )}
                    ...
                    {/* Footer: reazioni + commenti + share */}
                    <div className="flex items-center gap-3 mt-4 pt-3 border-t border-gray-100 flex-wrap">
                        ...
                        <div className="relative">
                            ...
                            {showReactions && (
                                <div
                                    className="absolute bottom-full left-0 mb-2 bg-white rounded-2xl shadow-lg border border-gray-100 flex gap-1 p-2 z-10">
                                    {REACTIONS.map((r) => (
                                        <button
                                            key={r.type}
